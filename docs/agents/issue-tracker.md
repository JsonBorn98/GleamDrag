...
- Triage 状态记录为每个 issue 文件顶部附近的 `Status:` 行（角色字符串见 `triage-labels.md`）。改状态用 `ticket.setStatus` 方法，不要手写 `Status:` 行——方法保证格式对齐 indexer 解析、校验合法枚举、触发 frontier/看板刷新。`claimed`/`resolved` 有专用方法 `ticket.claim`/`ticket.resolve` 优先用专用方法。这些方法不常驻在工具列表里（触发频率低于 wayfinder/domain，改用 `mcp__ext__invoke` 调用），字段照抄：
...
- 评论和对话历史追加到文件底部 `## Comments` 标题下——triage 场景用 `triage.appendComment` 方法（自动前置 AI 免责声明），其他场景直接 Write/Edit 追加。同样不常驻，走 `mcp__ext__invoke`：
  ...
  改依赖边（`Blocked by:` 行）用 `ticket.setBlockedBy` 方法，不要手改——方法保证三格式重写（不插出第二行冲突）、写前校验引用编号在同 feature 真实存在（悬空引用、引用自己都抛错）。空数组等于清依赖。删 ticket 用 `ticket.delete` 方法，不要裸 `rm`——按 feature+number 定位（打错编号不会误删）、被依赖时拒删并列出引用方（`force: true` 跳过校验，明确知道后果再用）。同样走 `mcp__ext__invoke`：

  ```jsonc
  {
    "name": "matt-flow",
    "method": "ticket.setBlockedBy", // 或 "ticket.delete"
    "args": {
      "workspace": "<项目根绝对路径>",
      "feature": "<所属分组>",
      "number": "<ticket 编号，如 01>",
      "blockedBy": ["02"] // setBlockedBy 用：依赖编号列表，空数组清依赖；delete 用 "force": false
    }
  }
  ```

  `ticket.create` 支持可选 `type` 字段（`research` / `prototype` / `grilling` / `task`）：传入时创建 wayfinder 决策票（正文自动写 `## Question` 模板 + `Type:` 行），不传创建 to-tickets 实现票。

  这些方法缺字段都会抛"缺必填字段 X"，照 reason 补齐重试即可，不会静默把 `undefined` 写进文件。
...
- **Map**，`.scratch/<effort>/map.md`，正文五个区：Destination（走到尽头是什么样）、Notes（领域/技能/常驻偏好）、Decisions so far（已关闭 ticket 的决策索引）、Not yet specified（战争迷雾，能感知但还无法 ticket）、Out of scope（范围外，永不毕业）。灯塔/暗礁去容器化（ADR-0015）后不再是 map.md 分区——改写 `lighthouses.md` / `shoals.md` 专属文件，存量老图分区照常读取。用 `create_wayfinder_map` 工具创建骨架（保证五区标题完整，indexer 按这些 `##` 标题解析），不要手写整个 map.md。map.md 已存在但缺几个分区（早期版本、或不是 create_wayfinder_map 建的旧图）时，用 `backfill_map_sections` 工具补齐——已有分区和内容一字不动，只补缺的空区；不要手写 map.md 去补分区标题，那样容易把内容写进错误的区。
